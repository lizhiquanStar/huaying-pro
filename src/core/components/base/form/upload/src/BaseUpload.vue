<template>
  <div
    class="base-upload"
    :class="{
      'is-file': Config.type === 'file' && !Config.video
    }"
  >
    <!-- 图片视频列表 -->
    <vuedraggable
      ...
      v-model="uploads"
      v-if="!Config.type || Config.video || Config.type === 'image'"
    ...
    >
      ...
      >
        <el-image
          :src="imgShowUrl + item.url"
          :preview-src-list="getBigImageList(index)"
          :ref="index"
          v-if="!Config.video"
        ></el-image>
        <video :src="item.url" style="width:120px;height:120px;" v-else></video>
        ...
        <span class="base-upload__image-actions">
          ...
          <i
            class="el-icon-zoom-in base-upload__action-item"
            @click.stop="showBigVideo(item.url)"
            v-else
          ></i>
          <i
    ...
    >
      <span
        v-if="
          (!Config.type || Config.type === 'image' || Config.video) &&
      ...
      >
        ...
        <img v-else-if="Config.video" :src="shipinImg" style="border:none" />
        <i
          v-else
      ...
      <el-button
        v-if="Config.type === 'file' && !Config.video"
...
import shipinImg from '@/assets/images/component/shipin.png';
...
export default class BaseUpload extends Mixins(FromComponentFactory) {
  ...
  uploads: UploadItem[] = [];
  imgShowUrl = API_CONFIG.imgShowUrl; //图片前缀域名
  shipinImg = shipinImg;
  ...
  getBigImageList(index: number) {
    const imageUploads = this.uploads.map((i) => this.imgShowUrl + i.url);
    if (index !== 0) {
