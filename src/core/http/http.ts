  ...
  try {
    ...
    switch (method) {
      case HTTP_METHODS.GET:
        res = await http.get(
          (!apiUrlNo ? API_CONFIG.apiPrefixUrl : API_CONFIG.apiPrefixUrl1) +
            url,
          {
            ...CancelToken,
            params,
      ...
      default:
        res = await http.get(
          !apiUrlNo ? API_CONFIG.apiPrefixUrl : API_CONFIG.apiPrefixUrl1 + url,
          {
            ...CancelToken,
            paramsSerializer,
            params,
    ...
    if (res.data.errno === API_CONFIG.successCode) {
      API_CONFIG.successHandle && (await API_CONFIG.successHandle());
      return Promise.resolve(
        (res.data as any).data || API_CONFIG.successMessage
      );
    } else {
