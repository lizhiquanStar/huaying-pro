...
export class Common {
  ...
  currentCloums: any = [];
  listArr: any[] = [];
  arrComponentIds = ['4', '6', '12', '13', '14', '15', '17'];
  ...
  async formType(
    ...
    switch (item.componentId) {
      ...
      case '18': {
        const res: any = await TrendTableApi.treeList();
        return {
          label: item.name,
          modelName: item.code,
          type: this.formItemType[item.componentId],
          ...
          treeSelectConfig: {
            ...
            tree: {
              ...
              defaultExpandedKeys: formMoudle[item.code]
                ? [formMoudle[item.code]]
                : [],
              nodeKey: 'id',
              props: {
                label: 'departName',
    ...
    this.listArr = await Promise.all(
      res.map(async (item: any) => {
        ...
        cloums.push({
          ...
          // width: item.componentId === '14' || item.componentId === '15' || item.componentId === '17' ? '140px' : '',
          prop:
  ...
  async analysisTable(data: any) {
    ...
    // var codeImg = this.listArr.find((item) => item.componentId === '17');
    // console.log(codeImg);
    // if (codeImg && data[codeImg.code]) {
    //   data[codeImg.code] = data[codeImg.code].split(',')
    // }
    var name: any = '';
    var codeFile = this.listArr.find((item) => item.componentId === '15');
    if (codeFile && data[`${codeFile.code}Name`]) {
      const res = await TrendTableApi.fileInfos({
        ids: data[`${codeFile.code}Name`]
      });
